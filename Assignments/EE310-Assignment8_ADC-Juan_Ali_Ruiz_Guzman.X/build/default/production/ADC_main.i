...
void LCD_Init()
{
    ...
    LCD_String_xy(1,0," Juan Ali Ruiz ");
...
void __attribute__((picinterrupt(("irq(default),base(0x4008)")))) my_ISR(void);
...
void ADC_RA2Conver(void){
    ...
    dB = 20 * log10f((float)digital * ((float)3.3 / 4096.0));


    sprintf(dBStr,"%.2f",dB);
    strcat(dBStr,"   dB ");

    LCD_String_xy(1,0,"Input Sound: ");

    LCD_String_xy(2,0,dBStr);
...
void __attribute__((picinterrupt(("irq(7),base(0x4008)")))) ISR(void)
{
    if (PIR0bits.IOCIF == 1){
        if (IOCAFbits.IOCAF0)
        {
...
{
    unsigned int i;
    unsigned long delay_us = 1000000UL / frequency / 2;
